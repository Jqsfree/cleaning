#!/usr/bin/env python3
"""exo_outdoor 标题句向量否决器：MiniLM + LR。

用途：排除 title/channel **与户外探险无关** 的条目（游戏/MV/播客/开箱等）。
不用于 POV/画面类 F（人工标里 131/132 个 F 标题仍像户外，不能当负例）。
...
MODEL_PATH = PROJECT / "models/exo_outdoor_text_clf_f.pkl"
CALIB_PATH = PROJECT / "models/exo_outdoor_text_clf_f_calibration.json"
DEFAULT_QC = (
    Path("/home/jqs/tmp/exo户外探险_975f0137_qc_result.csv"),
    Path("/home/jqs/tmp/户外探险_979680ae_qc_result.csv"),
)
DEFAULT_HARD_NEGATIVE_GLOB = "data/runs/exo_outdoor/**/05_clean/**/*drop*.csv"
...
def make_pipeline() -> Pipeline:
    MiniLMEncoder.__module__ = "exo_outdoor_text_classifier"
    return Pipeline([
        ("emb", MiniLMEncoder()),
...
def train_and_calibrate(
    ...
    *,
    ...
    MiniLMEncoder.__module__ = "exo_outdoor_text_classifier"
    sys.modules["exo_outdoor_text_classifier"] = sys.modules[__name__]
    ...
    result = {
        ...
        "notes": [
            "正例=人工 T（户外相关标题）；负例=blacklist drop（标题确定非户外）",
            "不训 POV/画面 F；MiniLM 只排除 title/channel 与户外无关",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="exo_outdoor 标题无关否决器（MiniLM）")
