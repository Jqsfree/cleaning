    ...
    *,
    ...
    def flush() -> None:
        if not pending:
            return
        tensors = [
            pil_to_tensor(image).float().div(255).to(device)
            for _, _, image in pending
        ]
        with torch.inference_mode():
            outputs = model(tensors)
        for (source_index, video_id, image), output in zip(
            pending, outputs, strict=True,
        ):
            keep = (output["labels"] == 1) & (
                output["scores"] >= score_threshold
            )
    ...
    for source_index, row in chunk.iterrows():
        ...
        try:
            ...
            error = (
                ...
                else f"decode_error:{type(exc).__name__}"
            )
            result = classify_thumbnail_person(
                None,
                frame_size=None,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="真人直播缩略图人体面积过滤")
    parser.add_argument("input")
    parser.add_argument("-o", "--output-dir", required=True)
    parser.add_argument(
        "--cache-dir",
        default="qc_thumb_cache/exemplar_sim",
    )
    ...
    if len(features) != len(source):
        raise RuntimeError(
            f"特征行数不守恒: features={len(features)} source={len(source)}",
        )
    if not features["video_id"].astype(str).equals(
        source["video_id"].astype(str),
    ):
        raise RuntimeError("特征与输入 video_id 顺序不一致")
    ...
    drop = enriched[
        enriched["thumb_person_action"].eq("highconf_drop")
    ].copy()
    remain = enriched[
        ~enriched["thumb_person_action"].eq("highconf_drop")
    ].copy()
    drop.to_parquet(output / "highconf_drop.parquet", index=False)
    remain.to_parquet(output / "remain.parquet", index=False)
    ...
    summary = {
        ...
        "drop": len(drop),
        "errors_kept": int(
            enriched["thumb_person_action"].eq("keep_error").sum()
        ),
        "actions": enriched["thumb_person_action"].value_counts().to_dict(),
        "reasons": enriched["thumb_person_reason"].value_counts().to_dict(),
        "score_threshold": args.score_threshold,
        "min_person_area": args.min_person_area,
        "model": "fasterrcnn_mobilenet_v3_large_320_fpn",
        "device": str(device),
        "elapsed_sec": round(elapsed, 1),
        "remain_path": str(output / "remain.parquet"),
        "drop_path": str(output / "highconf_drop.parquet"),
    }
    write_json(output / "summary.json", summary)
    write_json(output / "progress.json", {"status": "complete", **summary})
