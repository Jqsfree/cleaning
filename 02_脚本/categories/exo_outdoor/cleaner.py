#!/usr/bin/env python3
"""
categories/exo_outdoor/cleaner.py — exo 户外探险文本黑名单（certain-noise）

不注册到 pipeline/02_clean.py（品类默认仅 QC）。直接调用:

  PYTHONPATH=02_脚本 python - <<'PY'
  from categories.exo_outdoor.cleaner import clean
  clean(
      "data/runs/exo_outdoor/machine_0810/01_quality/…quality….csv",
      output_dir="data/runs/exo_outdoor/machine_0810/05_clean/run01/",
      raw_name="户外探险_merged_dedup_0810",
  )
  PY
"""
...
def clean(
    ...
    **kwargs,
    ...
    log("exo_outdoor 文本黑名单清洗（certain-noise）...")
