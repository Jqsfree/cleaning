...
import sys, os, re, time, argparse, textwrap, csv
...
def _reader(path: str) -> str:
    """根据文件扩展名返回 DuckDB reader 函数名"""
    ext = os.path.splitext(path)[1].lower()
    if ext in ('.csv', '.tsv'):
        return f"read_csv_auto('{path}', header=true, all_varchar=true, sample_size=-1, ignore_errors=true)"
    return f"read_parquet('{path}')"


def _reader_csv(path: str) -> str:
    """CSV 专用 reader（结构探测用）"""
    return f"read_csv_auto('{path}', header=true, all_varchar=true, sample_size=-1, ignore_errors=true)"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="SOP Phase 2: 抽样质检（支持公式计算 + 分层）",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent("""\
            示例:
              python3 phase2_sample.py baseline.parquet -o runs/002_audit/
              python3 phase2_sample.py keep.parquet -o runs/005_clean/run01/keep_qc/ --margin 0.03
              python3 phase2_sample.py drop.parquet -o runs/005_clean/run01/drop_qc/ --stratify
        """),
    )
    ...
    n_total = con.execute(
        f"SELECT COUNT(*) FROM {_reader(args.input)}"
    ).fetchone()[0]
    ...
    if args.stratify:
        ...
        con.execute(f"""
            ...
            SELECT * FROM {_reader(args.input)} LIMIT 0
        """)
        ...
        for keyword, kw_total, alloc in strata:
            ...
            con.execute(f"""
                ...
                SELECT * FROM (
                    SELECT * FROM {_reader(args.input)}
    ...
    clean_stem = re.sub(r'_(keep|drop)$', '', raw_stem)
    out_name = f"{clean_stem}_{sample_type}_qc.parquet"
    ...
    mark_done(out_dir, 2, samples=n_samples, total=n_total,
