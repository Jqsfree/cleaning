...
def main():
    ...
    cols = [c[0].lower() for c in con.execute(f"DESCRIBE SELECT * FROM {reader} LIMIT 0").fetchall()]
    has_label = label_col.lower() in cols
    has_cat = cat_col.lower() in cols
    has_keyword = "keyword" in cols
    has_channel = "channel" in cols

    if not has_label:
        log(f"  [WARN] 无 {label_col} 列")
        con.execute(f"CREATE TEMP TABLE data AS SELECT *, '' AS {label_col} FROM {reader}")
    else:
        con.execute(f"CREATE TEMP TABLE data AS SELECT * FROM {reader}")
    ...
    if has_keyword and non_lang > 0:
        ...
        """).fetchall()
    elif non_lang > 0 and not has_keyword:
        log("  [WARN] 无 keyword 列，跳过 keyword 污染统计")

    ch_rows = []
    if has_channel and non_lang > 0:
        ...
        """).fetchall()
    elif non_lang > 0 and not has_channel:
        log("  [WARN] 无 channel 列，跳过 channel 污染统计")
    ...
    lines.append(f"然后重新运行 02_clean.py 迭代清洗")
